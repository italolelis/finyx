...
This command writes ONLY to `.finyx/insights-config.json` (allocation mapping persistence) — it never modifies `.finyx/profile.json`.
...
Profile data is at `.finyx/profile.json`. Reference docs are at `~/.claude/finyx/references/insights/`.
...
Profile data is at `.finyx/profile.json`. Reference docs are at `~/.claude/finyx/references/`.
...
Assemble the unified report from the three agent outputs. Follow the section order exactly (D-01): Summary → Health Dashboard → Recommendations → Detail.
...
If a Task response does not contain the expected XML-tagged output, note the failure inline in Section 4 with: "Agent output not received — [agent name] did not return expected <[tag]> block. Re-run /fin:insights to retry."
...
This command writes ONLY to `.finyx/insights-config.json` (allocation mapping persistence on first run). It never modifies `.finyx/profile.json`.

## Agent Statefulness

All three specialist agents are stateless — they read profile data and reference docs, then return structured output. The orchestrating command (`/fin:insights`) handles all Write operations.
...
The legal disclaimer is emitted in Phase 2, BEFORE any financial data or advisory content. This differs from `/finyx:tax` and `/finyx:invest` which append the disclaimer at the end. `/fin:insights` must emit it first.
